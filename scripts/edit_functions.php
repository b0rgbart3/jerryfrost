...
function remove_this_painting($id) {
      ...
      if (file_exists($file_path)) {
         ...
         // fclose($file_path);
          ...
          $paintingNumber = 0;
          $paintingCount = count($json_data['paintings']);
       //   echo "<br>Looking for: ".$id;
          $found = false;

          foreach($json_data['paintings'] as $artwork) {
              
            
           // echo "<br>Comparing to: ".$artwork['id'];
           
                if ($artwork['id'] === $id) {
                    $found = true;
                    // echo "<br> Found artowrk id: ". $id;
                    // echo "<br> So we are not adding it to the array.<br>";
                }
                else {
                //    echo "<br>Not a match: ". $id;
                    $json_line = "    { \"id\":  \"" . $artwork['id']. "\", \"title\": \"".$artwork['title']."\", ";
                    $json_line = $json_line . "\"width\": \"".$artwork['width']."\", ";
                    $json_line = $json_line . "\"height\": \"".$artwork['height']."\", ";
                    $json_line = $json_line . "\"month\": \"".$artwork['month']."\", ";
                    $json_line = $json_line . "\"day\": \"".$artwork['day']."\", ";
                        ...
                        if ($artwork['sold']) {
                        $json_line = $json_line ."], \"sold\": true";} else {
                            $json_line = $json_line ."], \"sold\": false";
                        }
                     ...
                     //  echo "<br>Painting Number: ". $paintingNumber;
        
                            if ($paintingNumber < $paintingCount-2) {
                      ...
                      //  echo "About to push: " . $json_line . ", to: ", $json_lines ."<br>";
                        array_push($json_lines, $json_line);
                        $paintingNumber++;
                }
            } // end of artwork look
            // if (!$found) {
            //     echo "<br>DID NOT FIND the artwork to remove.";
            // } else {
            //     echo "<br>FOUND THE ARTWORK TO REMOVE.";
            // }
 
      //    echo "Done looping through artwork list.<br>";
